...
搜索会先合并同一部位上的等价候选：同步装只比较同步后的有效属性，不比较不再生效的原始属性上限和孔位；非同步装则比较属性、上限和魔晶石孔。戒指为了满足不同 ID 约束，每类等价候选最多保留两枚。同一速度贡献下，若一个候选的所有伤害属性都不低于另一个，则提前删除后者。进入整套搜索前，求解器还会将具有明显压缩收益的两个部位提前组合并按属性总和去重。例如衣服提供 `5+10`、裤子提供 `10+5`，与两者属性对调后的组合会在接触其他部位前合并。随后搜索按组累计属性，并继续合并累计属性完全相同的状态。搜索还会根据剩余部位可提供的最小、最大速度提前排除无法到达目标的状态，并对最后一组候选直接流式评分，避免在内存中保存全部最终组合。计算在 Web Worker 中执行，不会阻塞页面；完成后可以预览结果并一键应用。
