...
* 先输入目标技速或咏速，仅在装备、魔晶石和当前食物全部生效后的最终速度精确等于目标值时保留方案。
...
搜索会先合并同一部位上的等价候选：同步装只比较同步后的有效属性，不比较不再生效的原始属性上限和孔位；非同步装则比较属性、上限和魔晶石孔。唯一品与非唯一品分别判定等价；戒指每类等价候选最多保留两枚，以便唯一品仍可组成合法配对。同一速度贡献下，若一个候选的所有伤害属性都不低于另一个，则提前删除后者。进入整套搜索前，求解器会根据其他搜索组的最小、最大速度反复删除不可能达到目标速度的候选，并将具有明显压缩收益的两个部位提前组合。例如衣服提供 `5+10`、裤子提供 `10+5`，与两者属性对调后的组合会在接触其他部位前合并。随后搜索按组累计属性，合并累计属性完全相同的状态，并在每一层对相同速度的累计状态执行同样的全属性支配剪枝。搜索还会根据剩余搜索组的速度范围提前排除无法到达目标的状态。最终评分阶段会为累计状态和最后一组候选分别建立属性上界树；当两棵子树能够提供的各项最高属性仍无法超过当前最优伤害时，直接跳过两棵子树的整个笛卡尔积。上述剪枝只删除最终伤害不可能更高的状态，不依赖当前暴击等属性的边际收益。计算在 Web Worker 中执行，不会阻塞页面；完成后可以预览结果并一键应用。
